...
$sites = $DB->query("select * from homepage_links where $whereClause `id` = $id limit 1;");
...
$site_groups = $DB->query("SELECT * FROM `homepage_groups` WHERE `group_id` = $group_id  LIMIT 1");
...
if (!empty($site['link_desc']) && !empty($site['link_keywords'])) {
    ...
    if (!empty($sets)) {
        $DB->query("UPDATE `homepage_links` SET " . implode(', ', $sets) . " WHERE `id` = " . $id);
    }
}
if (empty($site["icon"])) {
...
$group = $DB->fetch($DB->query("select `group_name`,`group_icon` from homepage_groups where group_id=" . $site['group_id'] . " limit 1"));
