# Homepage（homepage spage）主题生成提示词

> 用途：把本文件作为提示词，投喂给任意 AI 编程工具（如 WorkBuddy / ChatGPT / Claude / 通义灵码等），
> 让它基于官方开发包规范，生成一个**可运行、合规、且与官方 `dev-theme` 风格截然不同**的Homepage新主题。
...
你是Homepage（homepage spage，开源导航网站程序）的主题开发助手。请根据**「一、风格输入区」**中用户给出的风格描述，生成一个完整、可直接启用、零框架依赖的新主题。
...
```json
{
    ...
    "theme_name": "目录名(如 homepagelight)",
    "theme_version": "1.0.0",
    "theme_explain": "主题一句话说明",
    "theme_demo": "演示地址,如(https://spage.homepage.com/theme/homepagelight)",
    "requires": ">=1.2.5",
    "updated_at": "2026-01-01",
    "license": "MIT",
    "theme_course": "主题文档(https://spage.homepage.com/doc/homepagelight)"
}
```
...
> - `homepage_sou` 表只存了 `sou_link` / `sou_waplink`（搜索跳转地址），**没有**联想词 API 字段；
> - 站内链接集中在 `homepage_links` 表（字段：`name` `url` `icon` `link_desc` `link_keywords` `link_status` `link_pwd`），需主题自行查询。
...
**方案 B — 服务端查询（覆盖全量）**
- 在 `functions.php` 新增 `theme_search_links($kw)`（`theme_` 前缀，加 `function_exists()` 守卫避免重复定义），用全局 `$DB` 查 `homepage_links`：
  ```php
  function theme_search_links($kw) {
      ...
      $like = "'%" . $esc . "%'";
      $sql = "SELECT `name`,`url`,`icon`,`link_desc` FROM `homepage_links` "
