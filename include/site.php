...
class SITE extends DB
{
    ...
    public function getGroups()
    {
        ...
        return $this->query("SELECT * FROM `homepage_groups` WHERE `group_status` = 1 " . $condition_str . " ORDER BY `group_order` ASC");
    }
    ...
    public function getCategorys($group_id)
    {
        ...
        return $this->query("SELECT * FROM `homepage_groups` WHERE `group_status` = 1 AND `group_id` = " . $group_id . " LIMIT 1");
    }
    ...
    {
        $group_id = (int)$group_id;
        return $this->query("SELECT * FROM `homepage_links` WHERE `group_id` = " . $group_id . " ORDER BY `link_order` ASC");
    }
    ...
    public function getLink($link_id)
    {
        $link_id = (int)$link_id;
        // 修复SQL错误: ADN -> AND
        return $this->get_row("SELECT * FROM `homepage_links` WHERE `id` = " . $link_id . " AND `link_pwd` = 0");
    }
    ...
    {
        return $this->query("SELECT * FROM `homepage_tags` ORDER BY `homepage_tags`.`sort` ASC");
    }
    ...
    {
        return $this->query("SELECT * FROM `homepage_sou` WHERE `sou_st` = 1 ORDER BY `homepage_sou`.`sou_order` ASC");
    }
}
...
// 初始化全局变量
$conf = isset($conf) ? $conf : (isset($GLOBALS['conf']) ? $GLOBALS['conf'] : array());
...
//cookie拦截规则
$cookiefilter = "benchmark\s*?\(.*\)|sleep\s*?\(.*\)|load_file\s*?\\(|\\b(and|or)\\b\\s*?([\\(\\)'\"\\d]+?=[\\(\\)'\"\\d]+?|[\\(\\)'\"a-zA-Z]+?=[\\(\\)'\"a-zA-Z]+?|>|<|\s+?[\\w]+?\\s+?\\bin\\b\\s*?\(|\\blike\\b\\s+?[\"'])|\\/\\*.*\\*\\/|<\\s*script\\b|\\bEXEC\\b|UNION.+?SELECT\s*(\(.+\)\s*|@{1,2}.+?\s*|\s+?.+?|(`|'|\").*?(`|'|\")\s*)|UPDATE\s*(\(.+\)\s*|@{1,2}.+?\s*|\s+?.+?|(`|'|\").*?(`|'|\")\s*)SET|INSERT\\s+INTO.+?VALUES|(SELECT|DELETE)@{0,2}(\\(.+\\)|\\s+?.+?\\s+?|(`|'|\").*?(`|'|\"))FROM(\\(.+\\)|\\s+?.+?|(`|'|\").*?(`|'|\"))|(CREATE|ALTER|DROP|TRUNCATE)\\s+(TABLE|DATABASE)|\\bselect\\b\\s*@@|\\bselect\\b\\s+\\w+\\s*\\(\\s*\\)|\\b(and|or)\\b\\s*\\(\\s*select\\b";
