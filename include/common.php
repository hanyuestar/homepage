...
try {
    $web_config = $DB->query("SELECT * FROM `homepage_config`");
    ...
    if ($web_config === false) {
        ...
        if (defined('DEBUG') && DEBUG === true) {
            exit("<h3>Homepage 系统错误</h3><p>{$errorMsg}</p><p>数据库错误: {$dbError}</p>");
        } else {
    ...
    if (!$hasRows) {
        ...
        if (defined('DEBUG') && DEBUG === true) {
            exit("<h3>Homepage 系统错误</h3><p>{$errorMsg}</p>");
        } else {
