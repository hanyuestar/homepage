...
// 收集数据库中所有被引用的图标字段值
$icon_values = array();
$res = $DB->query("SELECT `group_icon` FROM `homepage_groups` WHERE `group_icon` IS NOT NULL AND `group_icon` != ''");
if ($res) {
...
$res = $DB->query("SELECT `icon` FROM `homepage_links` WHERE `icon` IS NOT NULL AND `icon` != ''");
if ($res) {
    while ($row = $DB->fetch($res)) {
            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p>扫描 <code>files/download</code> 与 <code>files/upload</code> 目录中的全部文件，并与数据库中 <code>homepage_groups</code> 表的 分组图标</code>、和 <code>homepage_links</code> 表的 链接图标</code> 进行比对，找出未被使用的文件。删除后不可恢复，请谨慎操作！</p>
                    </div>
