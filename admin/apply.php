 ...
 * @FilePath: /homepage/admin/apply.php
 * @Copyright (c) 2024 by Homepage, All Rights Reserved. 
 */
$title = '收录管理';
include './head.php';
$applyrows = $DB->num_rows($DB->query("SELECT * FROM `homepage_apply`"));
...
if ($set == 'edit_submit') {
    $id = intval($_GET['id']);
    $rows2 = $DB->query("select * from homepage_apply where apply_id='$id' limit 1");
    $rows = $DB->fetch($rows2);
    if (!$rows) {
    ...
    $sql = "UPDATE `homepage_apply` SET `apply_name` = '" . $name . "', `apply_group` = '" . $group . "',`apply_icon` = '" . $icon . "',`apply_url` = '" . $url . "' WHERE `homepage_apply`.`apply_id` = '" . $id . "';";
    if ($DB->query($sql)) {
        echo '<script>$.alert({title:"成功",content:"修改成功！",buttons:{confirm:{text:"确定",btnClass:"btn-primary",action:function(){window.location.href="./apply.php";}}}});</script>';
    } else {
...
<?php
    $id = intval($_GET['id']);
    $row2 = $DB->query("select * from homepage_apply where apply_id='$id' limit 1");
    $row = $DB->fetch($row2);
    if (!$row) {
...
<?php elseif ($set == 'conf'): ?>
    ...
    <form action="./apply.php?set=conf_submit" method="POST">
        ...
        <div class="form-group">
            ...
            <small class="help-block">显示在收录页的公告<code>使用HTML代码编写</code><br>工具：在线MD编辑器 编辑后复制html代码粘贴</small>
        </div>
        <div class="form-group">
                        ...
                        <span class="alert-help">已审核的图标会被隐藏，点击图标可重新加载<br>部分网站图标一直处于加载或无法显示，可能原因：无法访问或跨域问题，建议建将图标本地化</span>
                            ...
                            <div class="table-responsive" id="applylist">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
<?php
                                        $rs = $DB->query("SELECT * FROM `homepage_apply` ORDER BY `homepage_apply`.`apply_time` DESC");
                                        $i = 0;
...
elseif (preg_match("/^<svg*/", $res['apply_icon'])) { echo $res['apply_icon']; }
                                                ...
                                                <img class="lazy" src="/assets/admin/loading.gif" data-original="<?php echo htmlspecialchars($res['apply_icon'], ENT_QUOTES, 'UTF-8'); ?>">
<?php } ?>
<?php } else { ?>
                                                <img class="lazys" title="获取" src="/assets/admin/img/get.png" data-original="<?php echo htmlspecialchars($res['apply_icon'], ENT_QUOTES, 'UTF-8'); ?>">
<?php } ?>
                                            </td>
