        ...
        <div class="card">
          <div class="card-body">
            ...
            <?php if ($set == 'add'): ?>
              ...
              <h4>新增链接<a  class="btn"  href="./link.php"><i class="mdi mdi-chevron-left"></i>返回</a></h4>
              ...
              <form id="addLinkForm" action="./ajax_link.php?submit=add_link" method="POST">
                ...
                <div class="form-group">
                  ...
                  <textarea rows="2" class="form-control" id="add_desc" name="link_desc" maxlength="255" placeholder="链接描述"></textarea>
               ...
               <a href="./link.php"><i class="mdi mdi-chevron-left"></i>返回</a>
                ...
                <?php elseif ($set == 'edit'): ?>
                  ...
                  ?>
                   <h4>修改链接信息  <a  class="btn"  href="./link.php?page=<?php echo $page ?>"><i class="mdi mdi-chevron-left"></i>返回</a></h4>
                  ...
                  <form id="editLinkForm" action="./ajax_link.php?submit=edit_link&id=<?php echo $id; ?>" method="POST">
                    ...
                    <div class="form-group">
                      ...
                      <textarea rows="2" class="form-control" id="edit_desc" name="link_desc" maxlength="255" placeholder="仅部分主题支持，可不填"><?php echo htmlspecialchars($row['link_desc']); ?></textarea>
                      <small class="help-block">链接描述仅部分主题支持显示和详情页SEO，访问详情页时若为空将自动采集写入，采集失败写入"无"</small>
                    </div>
                  ...
                  <a href="./link.php?page=<?php echo $page ?>">
                    <i class="mdi mdi-chevron-left"></i>返回< /a>
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          try {
            ...
            // 成功
            if (resp.code == 200) {
              lightyear.notify(resp.msg, 'success', 1000);
              setTimeout(function() {
                location.reload();
              }, 1000);
              return;
            ...
            // 链接已存在，询问是否更新
            if (resp.code == 201 && resp.data && resp.data.id) {
              var existId = resp.data.id;
              $.alert({
                title: '提示',
                content: '该链接已存在，是否覆盖更新？',
                buttons: {
                  confirm: {
                    text: '更新',
                    ...
                    action: function() {
                      ...
                      $.ajax({
                        url: './ajax_link.php?submit=edit_link&id=' + existId,
                        method: 'POST',
                        ...
                        error: function() {
                          lightyear.loading('hide');
                          lightyear.notify('服务器错误', 'danger', 3000);
                        }
                      });
                    }
                  },
                  cancel: {
                    text: '取消',
                    action: function() {
                      lightyear.notify('已取消更新', 'info', 1500);
                    }
                  }
                }
              });
            ...
            lightyear.notify(resp.msg || '操作失败', 'danger', 3000);
          } catch (e) {
