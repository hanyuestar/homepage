...
switch ($submit) {
	...
	case 'add_tag':
		...
		if ($name == null or $link == null) {
			...
			$sql = "INSERT INTO `homepage_tags` (`tag_id`, `tag_name`, `tag_link`, `tag_target`,`sort`) VALUES (NULL, '" . $name . "', '" . $link . "', '" . $target . "','" . $sort . "');";
			if ($DB->query($sql)) {
	...
	case 'edit_tag':
		$id = intval($_GET['id']);
		$sort = intval($_POST['sort'] ?: 10);
		$rows2 = $DB->query("select * from homepage_tags where tag_id='$id' limit 1");
		$rows = $DB->fetch($rows2);
		if (!$rows) {
		...
		if ($name == null or $link == null) {
			...
			$sql = "UPDATE `homepage_tags` SET `tag_name` = '" . $name . "', `tag_link` = '" . $link . "', `tag_target` = '" . $target . "', `sort` = '" . $sort . "'  WHERE `homepage_tags`.`tag_id` = " . $id . ";";
			if ($DB->query($sql)) {
	...
	case 'add_link':
		...
		if ($name == null or $url == null) {
			...
			$url_check = rtrim($url, '/');
			$exists = $DB->get_row("SELECT `id` FROM `homepage_links` WHERE `url` = '$url_check' OR `url` = '$url_check/' LIMIT 1");
			...
			$sql = "INSERT INTO `homepage_links` (`id`, `name`, `group_id`, `url`, `icon`, `link_desc`, `link_keywords`, `link_order`) VALUES (NULL, '" . $name1 . "', '" . $group_id . "', '" . $url . "', '" . $icon . "', '" . $link_desc . "', '" . $link_keywords . "', '" . $link_order . "');";
			if ($DB->query($sql)) {
				$newid = $DB->insert_id();
	...
	case 'edit_link':
		$id = intval($_GET['id']);
		$rows2 = $DB->query("select * from homepage_links where id='$id' limit 1");
		$rows = $DB->fetch($rows2);
		if (!$rows) {
			json_response(404, '该条记录不存在！');
		}
		...
		if ($name == null or $url == null) {
			...
			$sql = "UPDATE `homepage_links` SET `name` = '" . $name1 . "', `link_desc` = '" . $link_desc . "', `link_keywords` = '" . $link_keywords . "', `url` = '" . $url . "', `icon` = '" . $icon . "', `group_id` = '" . $group_id . "', `link_pwd` = " . $link_pwd . " WHERE `homepage_links`.`id` = '" . $id . "';";
			if ($DB->query($sql)) {
	...
	case 'add_sou':
		...
		if (empty($name) && empty($alias) && empty($hint) && empty($link) && empty($color) && empty($icon)) {
			...
			$sql = "INSERT INTO `homepage_sou` (`sou_id`, `sou_alias`, `sou_name`, `sou_hint`, `sou_color`, `sou_link`, `sou_waplink`, `sou_icon`, `sou_st`, `sou_order`) VALUES
...
";
	...
	case 'edit_sou':
		$id = intval($_GET['id']);
		$rows2 = $DB->query("select * from homepage_sou where sou_id='$id' limit 1");
		$rows = $DB->fetch($rows2);
		if (!$rows) {
			json_response(404, '该条记录不存在！');
		}
		...
		if (empty($name) && empty($alias) && empty($hint) && empty($link) && empty($color) && empty($icon) && empty($order)) {
			...
			$sql = "UPDATE `homepage_sou` SET `sou_alias` = '" . $alias . "', `sou_name` = '" . $name . "', `sou_hint` = '" . $hint . "', `sou_color` = '" . $color . "', `sou_link` = '" . $link . "', `sou_waplink` = '" . $waplink . "', `sou_icon` = '" . $icon . "', `sou_st` = '" . $st . "', `sou_order` = '" . $order . "' WHERE `homepage_sou`.`sou_id` = " . $id . ";";
			if ($DB->query($sql)) {
	...
	case 'set_group':
		...
		foreach ($_POST['links'] as $lk => $lv) {
			$lv = intval($lv);
			$sql = "UPDATE `homepage_links` SET `group_id` = '" . $group_id . "' WHERE `homepage_links`.`id` = " . $lv . ";";
			if (!$DB->query($sql)) {
				$e++;
			}
		}
	...
	case 'allorder':
		...
		for ($i = 0; $i < count($_POST["link_array"]); $i++) {
			...
			$sql = "UPDATE `homepage_links` SET `link_order` = '" . $i . "' WHERE `homepage_links`.`id` = " . $link_id . ";";
			if (!$DB->query($sql)) {
				$e++;
			}
		}
	...
	case 'pwd_link':
		...
		foreach ($_POST['links'] as $lk => $lv) {
			$lv = intval($lv);
			$sql = "UPDATE `homepage_links` SET `link_pwd` = '" . $pwd_id . "' WHERE `homepage_links`.`id` = " . $lv . ";";
			if (!$DB->query($sql)) {
				$e++;
			}
		}
	...
	case 'on':
		...
		foreach ($_POST['links'] as $lk => $lv) {
			$lv = intval($lv);
			$sql = "UPDATE `homepage_links` SET `link_status` = '1' WHERE `homepage_links`.`id` = " . $lv . ";";
			if (!$DB->query($sql)) {
				$e++;
			}
		}
	...
	case 'off':
		...
		foreach ($_POST['links'] as $lk => $lv) {
			$lv = intval($lv);
			$sql = "UPDATE `homepage_links` SET `link_status` = '0' WHERE `homepage_links`.`id` = " . $lv . ";";
			if (!$DB->query($sql)) {
				$e++;
			}
		}
	...
	case 'del':
		...
		foreach ($_POST['links'] as $lk => $lv) {
			$lv = intval($lv);
			$sql = "DELETE FROM `homepage_links` WHERE `homepage_links`.`id` = " . $lv . ";";
			if (!$DB->query($sql)) {
				$e++;
			}
		}
	...
	//返回全站所有链接
	case 'all_links':
		$rs = $DB->query("SELECT `id`, `url`, `name` FROM `homepage_links` ORDER BY `id` ASC");
		$links = array();
		while ($r = $DB->fetch($rs)) {
	...
	case 'edit_form':
		...
		$id = intval($_GET['id']);
		$row2 = $DB->query("select * from homepage_links where id='$id' limit 1");
		$row = $DB->fetch($row2);
		if (!$row) json_response(404, '该条记录不存在！');

		$grouplists = array();
		$pwd_lists  = array();
		$gq = $DB->query("SELECT * FROM `homepage_groups`");
		while ($g = $DB->fetch($gq)) $grouplists[] = $g;
		$pq = $DB->query("SELECT * FROM `homepage_pwd`");
