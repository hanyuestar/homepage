...
// 读取全部分组
$groups = array();
$grouplists = $DB->query("SELECT * FROM `homepage_groups` ORDER BY `group_order` ASC, `group_id` ASC");
...
$bookmark_code = "javascript:(function(){var u=location.href,t=document.title;void(open('" . $bookmark_base . "?from=bm&url='+encodeURIComponent(u)+'&title='+encodeURIComponent(t),'homepage_quick_add','toolbar=yes,location=yes,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=no,copyhistory=yes,left=200,top=200,width=560,height=780'));})();";
?>
<!DOCTYPE html>
<html lang="zh-CN">
...
<head>
    ...
    <title><?php echo $mode === 'quick' ? '导入导航页 - ' : '批量导入链接 - ';
    ...
    <style>
        * {
        ...
        .homepage-bookmark-btn::after {
            content: "拖拽到收藏夹收藏或点击复制";
            font-size: 15px
        }

        .homepage-bookmark-btn:hover {
            background: #1d4ed8
        }

        .homepage-bookmark-tip {
            margin-top: 12px;
            font-size: 13px;
            color: #1abc9c;
            min-height: 20px
        ...
        .custom-toast.toast-error .toast-content {
            color: #991b1b;
        }

        .custom-toast.toast-error .toast-close {
            color: #fca5a5;
        }
        ...
        .custom-toast.toast-warning .toast-content {
            color: #92400e;
        }
    </style>
</head>
...
<body>
    ...
    <div class="card">
        <div class="card-header">
            <h3><?php echo $mode === 'quick' ? 'Homepage - 导入导航页' : 'Homepage - 批量导入链接'; ?></h3>
        ...
        <div class="card-body">
            ...
            <?php else: ?>
                ...
                <div id="bookmarkGuide">
                    ...
                    <a id="homepageBookmarkBtn" class="homepage-bookmark-btn" href="<?php echo htmlspecialchars($bookmark_code, ENT_QUOTES); ?>" title="导入导航页">导入导航页</a>
                    <div id="homepageBookmarkTip" class="homepage-bookmark-tip"></div>
        ...
        <?php if ($mode === 'quick'): ?>
            ...
            function fetchInfo() {
                ...
                getXhr('ajax_link.php?submit=geturl&url=' + encodeURIComponent(url), 15000).then(function(d) {
                    if (!d) {
                        showTip('获取失败，目标网站无法访问或防火墙限制，请手动填写', 'error');
                        return;
                    }
            ...
            document.getElementById('quickAddForm').addEventListener('submit', function(e) {
                ...
                postForm(this.action, new FormData(this)).then(function(txt) {
                    ...
                    if (resp && resp.code == 200) {
                        ...
                        showToast(resp ? resp.msg : (txt || '添加失败，请重试'), 'error');
                        btn.disabled = false;
                        btn.textContent = '添加链接';
                    }
                }).catch(function(err) {
                    showToast('提交失败：' + (err.message || '请检查网络连接'), 'error');
                    btn.disabled = false;
                    btn.textContent = '添加链接';
                });
            });

            if (<?php echo json_encode($url); ?>) {
                fetchInfo();
            }
        <?php else: ?>
            // ===== 批量导入模式 =====
            ...
            if (isMobile) {
                document.getElementById('bookmarkGuide').style.display = 'none';
            }

            var bmBtn = document.getElementById('homepageBookmarkBtn');
            var bmTip = document.getElementById('homepageBookmarkTip');
            ...
            function saveEdit(btn) {
                ...
                postForm('ajax_link.php?submit=edit_link&id=' + id, fd).then(function(txt) {
                    if (txt.indexOf('成功') > -1) {
                        ...
                        item.querySelector('.res-edit').textContent = '编辑';
                        form.classList.remove('show');
                        showToast('已保存修改', 'success');
                    } else {
                        showToast(txt || '保存失败，请重试', 'error');
                    }
                }).catch(function() {
                ...
                for (var w = 0; w < workerCount; w++) {
                    workers.push((async function() {
                        while (queue.length) {
                            ...
                            var raw = '';
                            try {
                                raw = await postForm('ajax_link.php?submit=add_link', fd);
                            } catch (e) {
                                raw = '';
                            }
                            ...
                            var isSkip = !ok && resp && resp.msg && resp.msg.indexOf('已存在') > -1;
                            var id = ok && resp.id ? resp.id : '';
                            var msg = resp && resp.msg ? resp.msg : raw;
