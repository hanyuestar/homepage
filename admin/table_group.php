...
$pwd_map = array();
$pq = $DB->query("SELECT `pwd_id`, `pwd_key` FROM `homepage_pwd`");
while ($p = $DB->fetch($pq)) {
    $pwd_map[$p['pwd_id']] = $p['pwd_key'];
}
// 预取各分组链接数（group_id => 数量）
$link_count_map = array();
$lq = $DB->query("SELECT `group_id`, COUNT(`id`) AS cnt FROM `homepage_links` GROUP BY `group_id`");
while ($l = $DB->fetch($lq)) {
    $link_count_map[$l['group_id']] = intval($l['cnt']);
}
$groupsrows = $DB->num_rows($DB->query("SELECT `group_id` FROM `homepage_groups`")); //分组数量
$rs = $DB->query("SELECT * FROM homepage_groups ORDER BY group_order ASC");
?>
