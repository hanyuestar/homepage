 ...
 * @FilePath: /homepage/admin/pwd.php
...
if ($set == 'add_submit') {
    ...
    $sql = "INSERT INTO `homepage_pwd` (`pwd_id`, `pwd_name`, `pwd_key`, `pwd_ps`) VALUES (NULL, '" . $pwd_name . "', '" . $pwd_key . "', '" . $pwd_ps . "');";
    if ($DB->query($sql)) {
...
if ($set == 'edit_submit') {
    ...
    $sql = "UPDATE `homepage_pwd` SET `pwd_name` = '" . $pwd_name . "', `pwd_key` = '" . $pwd_key . "', `pwd_ps` = '" . $pwd_ps . "' WHERE `homepage_pwd`.`pwd_id` = " . $id . ";";
    if ($DB->query($sql)) {
       exit('<script>window.location.href="./pwd.php";</script>');
    } else {
...
if ($set == 'delete') {
    $id = intval($_GET['id']);
    $delsql = "DELETE FROM `homepage_pwd` WHERE `homepage_pwd`.`pwd_id` = " . $id;
    if ($DB->query($delsql)) {
       exit('<script>window.location.href="./pwd.php";</script>');
    } else {
...
<?php
    $id = intval($_GET['id']);
    $pg = $DB->fetch($DB->query("SELECT * FROM `homepage_pwd` WHERE `pwd_id` = " . $id));
    if (!$pg) {
                        ...
                        <ul class="mb-0 mt-2">
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    ...
                                    <tbody>
<?php
                                        $pgs = $DB->query("SELECT * FROM `homepage_pwd`");
