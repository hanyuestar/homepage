...
if ($set == 'add_submit') {
    ...
    if ($icon !== '' && $icon[0] !== '<') {
        $icon = '<img src="' . $icon . '" class="homepage-group-img" />';
    }
    $pwd = intval($_POST['group_pwd']);
    $max_order = $DB->get_row("SELECT MAX(`group_order`) AS max_order FROM `homepage_groups`");
    $group_order = intval($max_order['max_order']) + 1;
    if ($name == NULL) {
        echo '<script>lightyear.notify("保存错误,请确保带星号的都不为空！", "danger", 3000);</script>';
    } else {
        $sql = "INSERT INTO `homepage_groups` (`group_id`, `group_name`, `group_icon`,`group_order`,`group_pwd`) VALUES (NULL, '" . $name . "', '" . $icon . "', '" . $group_order . "', '" . $pwd . "')";
        if ($DB->query($sql)) {
...
if ($set == 'edit_submit') {
    $id = intval($_GET['id']);
    $rows2 = $DB->query("select * from homepage_groups where group_id='$id' limit 1");
    $rows = $DB->fetch($rows2);
    if (!$rows) {
    ...
    if ($name == NULL) {
        ...
        $sql = "UPDATE `homepage_groups` SET `group_name` = '" . $name . "', `group_icon` = '" . $icon . "',`group_pwd` = '" . $pwd . "' WHERE `homepage_groups`.`group_id` = '" . $id . "';";
...
if ($set == 'del') {
    ...
    $delsql1 = 'DELETE FROM `homepage_links` WHERE group_id =' . $id;
    $delsql2 = 'DELETE FROM `homepage_groups` WHERE group_id=' . $id;
    $DB->query($delsql1);
    $DB->query($delsql2);
    exit();
}
if ($set == 'on') {
    $id = intval($_POST['group_id']);
    $DB->query("UPDATE `homepage_groups` SET `group_status` = '1' WHERE `homepage_groups`.`group_id` =" . $id);
    exit();
}
if ($set == 'off') {
    $id = intval($_POST['group_id']);
    $DB->query("UPDATE `homepage_groups` SET `group_status` = '0' WHERE `homepage_groups`.`group_id` =" . $id);
    exit();
}
if ($set == 'sort') {
    for ($i = 0; $i < count($_POST["groups"]); $i++) {
        $group_id = intval($_POST["groups"][$i]);
        $sql = "UPDATE `homepage_groups` SET `group_order` = '" . $i . "' WHERE `homepage_groups`.`group_id` = " . $group_id . ";";
        $DB->query($sql);
    }
    exit();
}
...
<?php
$id = intval($_GET['id']);
$row2 = $DB->query("select * from homepage_groups where group_id='$id' limit 1");
$row = $DB->fetch($row2);
if (!$row) {
