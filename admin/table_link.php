...
$grouplists = array();
$group_lists = array();
$grows = $DB->query("SELECT * FROM `homepage_groups` ORDER BY `group_order` ASC");
...
$rs = $DB->query("SELECT * FROM `homepage_links` WHERE `group_id` = " . $page . " ORDER BY `link_order` ASC");
$grouprows = $DB->num_rows($rs);
$linksrows = $DB->num_rows($DB->query("SELECT * FROM `homepage_links`"));
?>
  ...
  <div class="btn-group btn-group-xs">
    ...
    <a class="btn btn-success sort-godown" data-bs-toggle="tooltip" title="移到底部">
      <i class="mdi mdi-arrow-collapse-down"></i>
          ...
          <td class="homepage"><?php echo $res['name']; ?></td>
