...
$title = '搜索引擎设置';
include './head.php';
$sousrows = $DB->num_rows($DB->query("SELECT * FROM `homepage_sou`"));
...
if ($set == 'delete') {
    $id = intval($_GET['id']);
    $sql = "DELETE FROM homepage_sou WHERE sou_id='$id'";
    if ($DB->query($sql)) {
...
<?php
    $id = intval($_GET['id']);
    $row2 = $DB->query("select * from homepage_sou where sou_id='$id' limit 1");
    $row = $DB->fetch($row2);
    if (!$row) {
?>
...
<?php
                            $rs = $DB->query("SELECT * FROM `homepage_sou` ORDER BY `homepage_sou`.`sou_order` ASC");
