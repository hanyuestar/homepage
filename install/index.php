...
/**
 * @File:   index.php
 * @User:   Homepage <admin@homepage.com>
 * @Date:   2023-12-16
 * @Description: Homepage安装程序
 */
...
// 版权信息设置
$cfg_copyright = '© 2022-' . date("Y") . ' HOMEPAGE';
...
if ($s == 3) {
    ...
    if (isset($_POST['s']) && $_POST['s'] == 3) {
        ...
        $admin_pwd_md5 = md5('homepage' . $admin_pwd);
        $safe_admin_user = mysqli_real_escape_string($con, $admin_user);
        if (!mysqli_query($con, "UPDATE `homepage_config` SET `v` = '$safe_admin_user' WHERE `k` = 'admin_user'")) {
            insError('设置管理员账号失败：' . mysqli_error($con));
        }
        if (!mysqli_query($con, "UPDATE `homepage_config` SET `v` = '$admin_pwd_md5' WHERE `k` = 'admin_pwd'")) {
            insError('设置管理员密码失败：' . mysqli_error($con));
        }
        $rowUser = mysqli_fetch_assoc(mysqli_query($con, "SELECT `v` FROM `homepage_config` WHERE `k` = 'admin_user'"));
        $rowPwd = mysqli_fetch_assoc(mysqli_query($con, "SELECT `v` FROM `homepage_config` WHERE `k` = 'admin_pwd'"));
...
// 检测目标数据库是否已存在 homepage_ 前缀的表（用于安装前提示用户）
if ($s == 'checktables') {
    ...
    // 查询指定库中 homepage_ 前缀的表
    $tables = [];
    if (mysqli_select_db($con, $dbname)) {
        $res = mysqli_query($con, "SHOW TABLES LIKE 'homepage_%'");
        if ($res) {
