...
setup_database() {
    ...
    local db_user="${MYSQL_USER:-homepage}"
    local db_pass="${MYSQL_PASSWORD:-homepage123456}"
    local db_name="${MYSQL_DATABASE:-homepage}"
...
show_info() {
    echo ""
    echo -e "${GREEN}=========================================="
    echo -e "  Homepage 导航页安装完成！"
    echo -e "==========================================${NC}"
    echo ""
    echo -e "  ${BLUE}前台地址:${NC} http://localhost:8080/"
    echo -e "  ${BLUE}后台地址:${NC} http://localhost:8080/admin/"
    echo ""
